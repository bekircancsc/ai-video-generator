...
```bash
npm run start -- [--topic <topic>] [--niche <niche>] [--niche-file <file.md>] [--scenes <n>] [--payload <file.json>] [--series <prefix>] [--json] [--stage <dir>] [--no-audio] [--no-images] [--no-music] [--no-cover] [--no-loudness]
```
